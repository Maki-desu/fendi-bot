...
- `/rolechange setup role1:@RoleA role2:@RoleB message:"Choose the role that fits you best!"` lets a server manager choose up to five roles and write the message members see before choosing. Members use `/rolechange choose` to pick one; their previous role from the configured list is removed. The bot needs Manage Roles permission, and selectable roles must be below the bot's highest role. The setting resets when the bot restarts.
