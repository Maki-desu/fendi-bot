...
- `/reactions set reaction:🌸 keywords:hello,welcome channel:#general` makes Fendi react when a message contains any listed keyword. Use `/reactions add reaction:✨ keywords:anime` to add a separate rule, `/reactions status` to view rules, and `/reactions clear` to disable them. Requires Manage Server permission.
- `/anime recommend` gives a random anime recommendation with a picture.
- `/anime update channel:#anime-updates` enables hourly notifications for new anime episodes airing that day in the selected channel. It also checks immediately when configured. Use `/anime preview channel:#anime-updates` to send a preview without notifying everyone or marking episodes as sent. Requires Manage Server permission.
- `/tiktok state:on` automatically fetches TikTok videos posted in the server and sends them as playable Discord videos. Use `/tiktok state:off` to disable it. Requires Manage Server permission.
- `/translate state:on channel:#translations` enables automatic translation of non-English messages to English, sending results only to the selected channel. Use `/translate state:off` to disable it. Requires Manage Server permission. The setting is saved to disk.
...
- `/deleteonmessage state:on channel:#warning-room warning:"Please do not message here."` watches one channel, deletes each triggering message, sends a warning mentioning the author, then deletes the warning after five seconds. Use `/deleteonmessage state:off channel:#warning-room` to disable it. This requires Manage Server and Manage Messages permissions.
