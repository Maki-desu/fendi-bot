...
- `/anime update channel:#anime-updates` enables hourly notifications for new anime episodes airing that day in the selected channel, combined into one embedded message. It also checks immediately when configured. Use `/anime preview channel:#anime-updates` to send a preview without notifying everyone or marking episodes as sent. Requires Manage Server permission.
